...
from .controlNetDiffusionModels import ControlNetDiffusionModel as ControlNetModel
...
def loadWeightsFromSafeTensor(
        ...
        if legacy is True:
            ...
            for module in moduleName:
                ...
                print("Loaded %d safetensors weights for %s"%(len(module_weights) , module))
        else:
            ## Contemporary Mode
                ...
                for i , (key , perm ) in enumerate(PYTORCH_CKPT_MAPPING[module]):
                    if "in_proj" not in key:
                        ...
                        if module == "text_encoder":
                            ...
                            if "weight" in key:
                                # Get the in_proj.weight
                                originalWeight = safeTensorWeights['state_dict'][key].float().numpy()
                                ...
                                originalBias = safeTensorWeights['state_dict'][key].float().numpy()
