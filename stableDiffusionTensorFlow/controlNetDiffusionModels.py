...
class DiffusionModel(keras.Model):
    def __init__(
        ...
        latent = keras.layers.Input((img_height // 8, img_width // 8, 4), name = "LatentImage_Input")
        ...
        controlNetResults = [controlNet1, controlNet2, controlNet3, controlNet4, controlNet5, controlNet6, controlNet7, controlNet8, controlNet9, controlNet10, controlNet11, controlNet12, controlNet13]
...
class ControlNetDiffusionModel(keras.Model):
    def __init__(
        ...
        super().__init__([latent, t_embed_input, context, inputHint], outputs, name = name)
...
class DiffusionModelV2(keras.Model):
    def __init__(
        self, img_height, img_width, max_text_length, name = None
    ):
...
class PaddedConv2D(keras.layers.Layer):
    def __init__(
        ...
        self.conv2d = keras.layers.Conv2D(filters, kernel_size, strides = strides, name = name)
...
class SpatialTransformer(keras.layers.Layer):
    def __init__(
        ...
        if fully_connected:
            self.proj1 = keras.layers.Dense(num_heads * head_size, name = "proj_in1_fullyConnected")
        else:
...
class CrossAttention(keras.layers.Layer):
    def __init__(self, num_heads, head_size, **kwargs):
        super().__init__(**kwargs)
        self.to_q = keras.layers.Dense(num_heads * head_size, use_bias = False, name = "to_q")
        self.to_k = keras.layers.Dense(num_heads * head_size, use_bias = False, name = "to_k")
        self.to_v = keras.layers.Dense(num_heads * head_size, use_bias = False, name = "to_v")
...
def zeroConv(tensor, channels, name = None):
    layer = keras.layers.Conv2D(filters = channels, kernel_size = 1, padding = 'same', name = name)
    for weight in layer.weights:
        weight.assign(tf.zeros_like(weight))
