...
def loadTextEmbedding(
        ...
        for textEmbedding in textEmbeddings:
            ...
            textEmbeddingFile = torch.load(embeddingsPath + textEmbedding, map_location = "cpu")
            ...
            if "pt" in textEmbedding:
                ...
                stringToToken = textEmbeddingFile["string_to_token"] # Token assigned to vector
            ...
            if type(stringToToken) is dict:
                token = list(stringToToken.keys())[0] # Convert dictionary to a list and then pull the first value
            else:
                token = stringToToken
...
def loadTextEmbeddingWeight(
    ...
    for embedding in embeddings:
        if np.size(embedding.vector[0]) != columnLength:
            ...
            print(embedding.name,"not compatible with current version of Stable Diffusion")
            continue
