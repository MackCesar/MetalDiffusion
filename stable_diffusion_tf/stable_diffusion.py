...
# More supressed warnings from TensorFlow
...
## Tokenizer
from .clipTokenizer import SimpleTokenizer, LegacySimpleTokenizer
...
class StableDiffusion:
    ...
    def __init__(
        ...
        ### Step 3: Which version of Stable Diffusin ###
        ...
        ## Step 5.1: Were pyTorchWeights given? ##
        if ".ckpt" in pyTorchWeights: # Yes
            self.pyTorchWeights = pyTorchWeights
            pyTorchWeights = None
        else: # No, the user gave a pre-compiled model
            self.pyTorchWeights = None
        
        ## Step 5.2: Create empty models ##
        text_encoder, diffusion_model, decoder, encoder = CreateModels(
            self.imageHeight,
            self.imageWidth,
            preCompiled = pyTorchWeights, # If not None, then we're passing on Keras weights ".h5"
            legacy = legacy,
            addedTokens = self.textEmbeddings
        )
        
        ## Step 5.3 Create object/class variables that point to the compiled models
        self.text_encoder = text_encoder
        self.diffusion_model = diffusion_model
        self.decoder = decoder
        self.encoder = encoder

        ## Step 5.4: Load pytorch weights if pytorch weights were given
        ...
        if pyTorchWeights is None:
            ...
            loadWeightsFromPytorchCKPT(
                self,
                self.pyTorchWeights, # Which weights to load, in this case maybe all four models
                legacy, # Which version of Stable Diffusion
    ...
    def generate(
        ...
        if input_image is not None:
            ...
            if type(input_image) is str:
                ...
                # input_image = np.resize(input_image, (self.imageHeight, self.imageWidth, input_image.shape[2]))
    ...
    def encodeText(
        ...
        if legacy is True:
            ...
            phrase = [49406] + inputs + [49407] * (76 - len(inputs))
            ...
            phrase = np.zeros((len(allTokens), MAX_TEXT_LEN), dtype = np.int32)
...
def CreateModels(
    ...
    floorDividedImageHeight = imageHeight // 8
    floorDividedImageWidth = imageWidth // 8
    ...
    if legacy is True:
        ...
        if controlNet is False:
            ...
            # Create Diffusion model
            print("ControlNet not implemented yet! Running in normal diffusion")
            diffusion_model = DiffusionModel(
                imageHeight,
                imageWidth,
                MAX_TEXT_LEN
        ...
        if preCompiled is not None:
            ...
            textEncoderWeights = preCompiled + "/text_encoder.h5"
            diffusionModelWeights = preCompiled + "/diffusion_model.h5"
            imageEncoderWeights = preCompiled + "/encoder.h5"
            decoderWeights = preCompiled + "/decoder.h5"

            text_encoder.load_weights(textEncoderWeights)
            print("...Text Encoder weights loaded!")
            diffusion_model.load_weights(diffusionModelWeights)
            print("...Diffusion weights loaded")
            encoder.load_weights(imageEncoderWeights)
            print("...Image Encoder weights loaded!")
            decoder.load_weights(decoderWeights)
            ...
            for module in moduleName:
                ...
                for i , (key , perm ) in enumerate(PYTORCH_CKPT_MAPPING[module]):
                    ...
                    if 'state_dict' in pytorchWeights:
                        ...
                        weight = pytorchWeights[key].numpy()
                    if perm is not None:
